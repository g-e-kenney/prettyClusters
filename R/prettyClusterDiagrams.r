...
prettyClusterDiagrams <- function(imgGenesFile = imgGenesFile,
    ...
    if (autoColor == FALSE) {
        ## here the colors are in the annotationGuide file
        ## this assigns them, adds default colors for hypothetical proteins / mobile genetic elements / other if not defined
        ...
        ## if we added "hyp" in annotation and it wasn't assigned a color in the guide file (same for "mge", "other")
...
##        }
        ## this probably isn't necessary with the scale_fill_manual fix from 2024
        ## but it makes everything alphabetical again
        ...
        ## this will color the genes specified, but it will assign colors based on a palette
        ...
        ## getting the list of gene types from our dataset
        geneTypes <- unique(processed$gene)
        colorNum <- length(geneTypes)
        geneTypes <- geneTypes[stringr::str_order(geneTypes)]
        allGeneTypes <- geneTypes
        ## flagging  hypothetical proteins.  if this is ever null i am probably out of a job.
        notMe <- which(geneTypes =="hyp")
        ## flagging mobile genetic elements if defined
        notMeEither <- which(geneTypes == "mge")
        ## flagging everything else without a special category
        norMe <- which(geneTypes == "other")
        ## these cases get non-pretty colors so we'll generate the palette without them
        if (any(notMe>0, notMeEither>0, norMe>0)) {
            geneTypes <- geneTypes[-c(notMe,notMeEither,norMe)]
        }
        ## dealing with larger sets of genes of interest
        ...
        if (length(notMe)>=1) {
            geneColors <- append(geneColors,  "#FFFFFF")
            geneTypes <- append(geneTypes, "hyp")
        }
        if (length(notMeEither)>=1) {
            geneColors <- append(geneColors, "#858585")
            geneTypes <- append(geneTypes, "mge")
        }
        if (length(norMe)>=1) {
            geneColors <- append(geneColors, "#DEDEDE")
            geneTypes <- append(geneTypes, "other")
        }
        ## with the color values as a named vector, it should not be necessary to alphabetically sort them _but_...
        ## just in case something changes 
        alphaGeneTypes <- sort(geneTypes)
        for (k in 1:length(alphaGeneTypes)) {
          geneIdx <- which(geneTypes==alphaGeneTypes[k])
          if (k == 1) {
            alphaColors <- geneColors[geneIdx]
          } else {
            alphaColors <- append(alphaColors, geneColors[geneIdx])
          }
        }    
        finalColors <- alphaColors
        allGeneTypes <- alphaGeneTypes
    ...
    ## little function to color genes using our new palette.  note: must have named vector for color values to correctly match genes
    ...
    if (alignToCore == TRUE) {
        ## note that where there are 2+ GOIs, this just centers them as a least-bad option (it's looking for coreGeneName)
    ...
    ## formerly scale_fill_manual with values = finalColors but fixed to use scale_fill_genes
    ...
    ## this way you have a copy of the annotated dataset and can skip auto-annotation if desired if you're recoloring in the future
    if (subclusterDiagrams == TRUE)   {
        ...
        for (i in 1:length(clustList))  {
            ...
            ## (this is incidentally why we had to add the scalebar back now - weird consequences)
            ## we'll make a temporary minifunction for coloring according to the trimmed color/gene set
            ...
            scale_fill_subgenes <- function(...) {
                ggplot2:::manual_scale(
                          'fill', 
                          ## breaks = someGeneTypes,
